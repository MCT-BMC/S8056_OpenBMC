...
void update_flash_progress(int pecent, const char *version_id, int progress, std::string additionalInfo)
{
    if(progress){
        FILE * fp;
        char buf[512];
        std::string info = "\"" + additionalInfo + "\"";
        sprintf(buf, "flash-progress-update %s %d %s", version_id, pecent, info.c_str());
                          ...
                          uint8_t cmd, std::vector<uint8_t> *rBuf)
{
    ...
    if (ret < 0)
    {
        std::cerr << __func__ << ": Get byte value fail\n";
        return -1;
    }
    ...
    if (mfrSize.size() == 1)
    {
        if (mfrSize[0] == 0xFF)
        {
            std::cerr << __func__ << ": Unknown upload mode\n";
            return -1;
        }
    }
