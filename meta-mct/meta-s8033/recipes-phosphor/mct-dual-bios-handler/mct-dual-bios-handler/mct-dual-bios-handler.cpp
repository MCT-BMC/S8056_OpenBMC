...
int enableBiosStartTimer(bool disablePreTimer, bool disableEndTimer)
{
    ...
    if(disablePreTimer)
    {
        ...
        biosStartTimer.async_wait([&](const boost::system::error_code ec) {
            ...
            if(isPowerOn() && !isCurrentPostCodeStarted()){
    ...
    setPostCodeHandler(std::shared_ptr<sdbusplus::asio::connection>& systemBus)
{
    auto postCodeMatcherCallback = [&](sdbusplus::message::message &msg)
    {
        ...
        if(value == 0x26)
        {
            ...
            uint32_t registerBuffer = 0;
            uint32_t snoopingMask = 0x0000ffff;
            if(read_register(0x1E789094,&registerBuffer) < 0)
            {
                std::cerr<<"Failed to read SNPWDR register \n";
            }

            if((registerBuffer & snoopingMask) == 0xE326)
            {
        ...
        if(amdBistState && value != 0x26)
        {
            ...
            if(((registerBuffer & snoopingMask) != 0xE32C) && ((registerBuffer & snoopingMask) != 0xE32D))
            {
